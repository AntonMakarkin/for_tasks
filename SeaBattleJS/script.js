...
const game = {
    ...
    generateOptionsShip(shipSize) {
        ...
        if (direction) { //выставляем корабль по вертикали
            x = Math.floor(Math.random() * 10);
            y = Math.floor(Math.random() * (10 - shipSize));
        }
        else { //выставляем корабль по горизонтали
...
const fire = (event) => {
    ...
    if (target.classList.length > 0 || 
        target.tagName !== 'TD' || !game.shipCount ) return;
...
//придумать способо перезагрузки игры без перезагрузки страницы